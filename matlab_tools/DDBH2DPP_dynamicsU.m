...
% Code to check time evolution of occupations & g2 for N*N site 2D DDBH Positive P with spatial averaging (for use with spatially uniform parameters)
...
for nb = 1:nbins
    ...
    n_bin =  real(mean(mean(mean(alpha((S_bin*(nb-1)+1):S_bin*nb,:,:,:).*beta((S_bin*(nb-1)+1):S_bin*nb,:,:,:), 1), 3), 4));
    g2_bin = real(mean(mean(mean((alpha((S_bin*(nb-1)+1):S_bin*nb,:,:,:).*beta((S_bin*(nb-1)+1):S_bin*nb,:,:,:)).^2, 1), 3), 4))./n_bin.^2;
