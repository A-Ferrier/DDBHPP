...
% Code to check time evolution of occupations & g2 for N unit cell (3*N site) 1D DDBH Lieb Positive P with spatial averaging (for use with spatially uniform parameters)
...
for nb = 1:nbins
    ...
    nA_bin =  real(mean(mean(alphaA((S_bin*(nb-1)+1):S_bin*nb,:,:).*betaA((S_bin*(nb-1)+1):S_bin*nb,:,:), 1), 3));
    g2A_bin = real(mean(mean((alphaA((S_bin*(nb-1)+1):S_bin*nb,:,:).*betaA((S_bin*(nb-1)+1):S_bin*nb,:,:)).^2, 1), 3))./nA_bin.^2;
    nB_bin =  real(mean(mean(alphaB((S_bin*(nb-1)+1):S_bin*nb,:,:).*betaB((S_bin*(nb-1)+1):S_bin*nb,:,:), 1), 3));
    g2B_bin = real(mean(mean((alphaB((S_bin*(nb-1)+1):S_bin*nb,:,:).*betaB((S_bin*(nb-1)+1):S_bin*nb,:,:)).^2, 1), 3))./nB_bin.^2;
    nC_bin =  real(mean(mean(alphaC((S_bin*(nb-1)+1):S_bin*nb,:,:).*betaC((S_bin*(nb-1)+1):S_bin*nb,:,:), 1), 3));
    g2C_bin = real(mean(mean((alphaC((S_bin*(nb-1)+1):S_bin*nb,:,:).*betaC((S_bin*(nb-1)+1):S_bin*nb,:,:)).^2, 1), 3))./nC_bin.^2;
