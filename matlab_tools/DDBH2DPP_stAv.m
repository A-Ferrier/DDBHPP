...
for nb = 1:nbins
    ...
    n_Av_bin =  real(mean(mean(mean(mean(alpha((S_bin*(nb-1)+1):S_bin*nb,init:end,:,:).*beta((S_bin*(nb-1)+1):S_bin*nb,init:end,:,:), 1), 2), 3), 4));
    g2_bin = real(mean(mean(mean(mean((alpha((S_bin*(nb-1)+1):S_bin*nb,init:end,:,:).*beta((S_bin*(nb-1)+1):S_bin*nb,init:end,:,:)).^2, 1), 2), 3), 4))./n_Av_bin.^2;
