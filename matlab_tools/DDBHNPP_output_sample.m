...
function [t, alpha, beta] = DDBHNPP_output_sample(N, Delta, U, F, J, tf, nsamples)

% Function for collecting all output data of many realisations of 
% DDBH_Nsite Positive P with Parameters U, F, Delta, J, 
% with nsamples realisations at time tf.  
    ...
    for nsample = 1:nsamples
        ...
        name_File=['file_output_' num2str(N) 'Site_Delta' correctnum2str(Delta) '_U' correctnum2str(U) '_F' correctnum2str(F) '_J' correctnum2str(J) '_tf' num2str(tf) '_Re' num2str(nsample) '.h5'];
