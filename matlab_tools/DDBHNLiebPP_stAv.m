...
% Code to check occupations & g2 for N unit cell (3*N site) 1D DDBH Lieb Positive P with time & spatial averaging
...
for nb = 1:nbins
    ...
    nA_bin =  real(mean(mean(mean(alphaA((S_bin*(nb-1)+1):S_bin*nb,init:end,:).*betaA((S_bin*(nb-1)+1):S_bin*nb,init:end,:), 1), 2), 3));
    g2A_bin = real(mean(mean(mean((alphaA((S_bin*(nb-1)+1):S_bin*nb,init:end,:).*betaA((S_bin*(nb-1)+1):S_bin*nb,init:end,:)).^2, 1), 2), 3))./nA_bin.^2;
    nB_bin =  real(mean(mean(mean(alphaB((S_bin*(nb-1)+1):S_bin*nb,init:end,:).*betaB((S_bin*(nb-1)+1):S_bin*nb,init:end,:), 1), 2), 3));
    g2B_bin = real(mean(mean(mean((alphaB((S_bin*(nb-1)+1):S_bin*nb,init:end,:).*betaB((S_bin*(nb-1)+1):S_bin*nb,init:end,:)).^2, 1), 2), 3))./nB_bin.^2;
    nC_bin =  real(mean(mean(mean(alphaC((S_bin*(nb-1)+1):S_bin*nb,init:end,:).*betaC((S_bin*(nb-1)+1):S_bin*nb,init:end,:), 1), 2), 3));
    g2C_bin = real(mean(mean(mean((alphaC((S_bin*(nb-1)+1):S_bin*nb,init:end,:).*betaC((S_bin*(nb-1)+1):S_bin*nb,init:end,:)).^2, 1), 2), 3))./nC_bin.^2;
