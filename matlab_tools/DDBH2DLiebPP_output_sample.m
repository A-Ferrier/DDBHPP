...
function [t, alphaA, alphaB, alphaC, betaA, betaB, betaC] = DDBH2DLiebPP_output_sample(N, Delta, U, F, J, tf, nsamples)

% Function for collecting all output data of many realisations of
% DDBH_Lieb_2D Positive P with Parameters U, F, J, Delta, with nsamples realisations at time tf.  

    %Loop over realisations to read
    for nsample = 1:nsamples
    
        %Read data from h5 file
        name_File=['file_output_Lieb_' num2str(N) 'x' num2str(N) '_Delta' correctnum2str(Delta) '_U' correctnum2str(U) '_F' correctnum2str(F) '_J' correctnum2str(J) '_tf' num2str(tf) '_Re' num2str(nsample) '.h5'];
        ...
        if nsample == 1
            alphaA = zeros(nsamples, length(t), N, N);
            betaA = zeros(nsamples, length(t), N, N);
            alphaB = zeros(nsamples, length(t), N, N);
            betaB = zeros(nsamples, length(t), N, N);
            alphaC = zeros(nsamples, length(t), N, N);
            betaC = zeros(nsamples, length(t), N, N);
        end
